...
binding.setCallBuffer(callBuffer.buffer);
...
//  const retBinTyp = mapBinType(ret);
  ...
  const retSize = sizes[retBinTyp];
  const argWriters = args.map(x => writers[mapRWType(x)]);
  const getReturnVal = readers[mapRWType(retBinTyp)];
